...
document.addEventListener('DOMContentLoaded', function () {
        ...
        try {
            ...
            if (EMAILJS_PUBLIC_KEY !== 'YOUR_PUBLIC_KEY' && typeof emailjs !== 'undefined') {
                try {
                    ...
                    console.warn('EmailJS Fehler (z.B. durch AdBlocker), nutze Fallback:', emailErr);
                }
            }

            // Wenn EmailJS fehlschlägt oder nicht konfiguriert ist -> Fallback zu FormSubmit
            if (!emailJsSuccess) {
                try {
                    const fallbackResponse = await fetch('https://formsubmit.co/ajax/elya.aftersales@gmail.com', {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON.stringify({
                            _subject: `[${formData.subject_text}] Support-Anfrage von ${formData.name}`,
                            Name: formData.name,
                            Email: formData.email,
                            Bestellnummer: formData.order_number,
                    ...
                    if (fallbackResponse.ok) {
                        ...
                        // Wenn alles API-basierte fehlschlägt -> Mailto
                        sendViaMailto(formData);
                    }
                } catch (fallbackError) {
                    console.warn('Fallback Netzfehler:', fallbackError);
                    sendViaMailto(formData);
                }
            }
        } catch (error) {
            console.error('Allgemeiner Fehler beim Formular:', error);
            setLoadingState(false);
            showFormError('Es gab einen unerwarteten Fehler. Bitte versuchen Sie es erneut oder verwenden Sie unsere E-Mail: elya.aftersales@gmail.com');
        }
    });
    ...
    function validateForm() {
        ...
        if (!privacy.checked) {
            showError(privacy, 'Sie müssen den Datenschutzbestimmungen zustimmen.');
            isValid = false;
        }
